...
:type: prop
:tags: line, triangle
:dependencies: I.47, II.7




.. picture:: II.13.graphic.inverted.png
...
.. container:: center
   ...
   the squares on ``CB``, ``BD`` are equal to twice the rectangle contained by ``CB``, ``BD`` and the square on ``DC``. [:ref:`II.7 <II.7>`]
...
But the square on ``AB`` is equal to the squares on ``BD``, ``DA``, for the angle at ``D`` is right; [:ref:`I.47 <I.47>`] and the square on ``AC`` is equal to the squares on ``AD``, ``DC``; therefore the squares on ``CB``, ``BA`` are equal to the square on ``AC`` and twice the rectangle ``CB``, ``BD``,
...
.. graphviz::
   ...
   digraph {
     ...
     "II.13" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/II/13/", target="_top"];
