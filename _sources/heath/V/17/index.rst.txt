...
.. picture:: V.17.graphic.inverted.png
...
Then, since ``GH`` is the same multiple of ``AE`` that ``HK`` is of ``EB``, therefore ``GH`` is the same multiple of ``AE`` that ``GK`` is of ``AB``. [:ref:`V.1 <V.1>`]

But ``GH`` is the same multiple of ``AE`` that ``LM`` is of ``CF``; therefore ``GK`` is the same multiple of ``AB`` that ``LM`` is of ``CF``.

Again, since ``LM`` is the same multiple of ``CF`` that ``MN`` is of ``FD``, therefore ``LM`` is the same multiple of ``CF`` that ``LN`` is of ``CD``. [:ref:`V.1 <V.1>`]
...
.. container:: center
   ...
   and ``KO`` is also the same multiple of ``EB`` that ``NP`` is of ``FD``, therefore the sum ``HO`` is also the same multiple of ``EB`` that ``MP`` is of ``FD``. [:ref:`V.2 <V.2>`]
...
.. graphviz::
   ...
   digraph {
     ...
     "V.17" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/V/17/", target="_top"];
     "V.17" -> "V.2";
     "V.17" -> "V.1";
   }
