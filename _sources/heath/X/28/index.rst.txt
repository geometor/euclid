:order: 28
:number: 405
:type: prop
:tags: line
:dependencies: II.6, IX.1, IX.24, IX.26, V.16, VI.12, VI.13, VI.16, VI.17, X.11, X.21, X.23, X.28.l.1




.. picture:: X.28.graphic.inverted.png

.. _X.28:

X.28
====

   To find medial straight lines commensurable in square only which contain a medial rectangle.

Let the rational straight lines A, B, C commensurable in square only be set out; let D be taken a mean proportional between A, B, [:ref:`VI.13 <VI.13>`] and let it be contrived that,


.. container:: center

   as B is to C, so is D to E. [:ref:`VI.12 <VI.12>`]


Since A, B are rational straight lines commensurable in square only, therefore the rectangle A, B, that is, the square on D [:ref:`VI.17 <VI.17>`], is medial. [:ref:`X.21 <X.21>`]

Therefore D is medial. [:ref:`X.21 <X.21>`]

And since B, C are commensurable in square only, and, as B is to C, so is D to E, therefore D, E are also commensurable in square only. [:ref:`X.11 <X.11>`]

But D is medial; therefore E is also medial. [:ref:`X.23 <X.23>`, addition]

Therefore D, E are medial straight lines commensurable in square only.

I say next that they also contain a medial rectangle.

For since, as B is to C, so is D to E, therefore, alternately, as B is to D, so is C to E. [:ref:`V.16 <V.16>`]

But, as B is to D, so is D to A; therefore also, as D is to A, so is C to E; therefore the rectangle A, C is equal to the rectangle D, E. [:ref:`VI.16 <VI.16>`]

But the rectangle A, C is medial; [:ref:`X.21 <X.21>`] therefore the rectangle D, E is also medial.

Therefore medial straight lines commensurable in square only have been found which contain a medial rectangle. Q. E. D.


.. _X.28.l.1:


**X.28.l.1**



To find two square numbers such that their sum is also square. 
Let two numbers AB, BC be set out, and let them be either both even or both odd. 
       
Then since, whether an even number is subtracted from an even number, or an odd number from an odd number, the remainder is even, [:ref:`IX.24 <IX.24>` :ref:`IX.26 <IX.26>`] therefore the remainder AC is even. 
Let AC be bisected at D. 
Let AB, BC also be either similar plane numbers, or square numbers, which are themselves also similar plane numbers. 
Now the product of AB, BC together with the square on CD is equal to the square on BD. [:ref:`II.6 <II.6>`] 
And the product of AB, BC is square, inasmuch as it was proved that, if two similar plane numbers by multiplying one another make some number the product is square. [:ref:`IX.1 <IX.1>`] 
Therefore two square numbers, the product of AB, BC, and the square on CD, have been found which, when added together, make the square on BD. 
And it is manifest that two square numbers, the square on BD and the square on CD, have again been found such that their difference, the product of AB, BC, is a square, whenever AB, BC are similar plane numbers. 
But when they are not similar plane numbers, two square numbers, the square on BD and the square on DC, have been found such that their difference, the product of AB, BC, is not square. Q. E. D.


.. _X.28.l.2:


**X.28.l.2**


To find two square numbers such that their sum is not square. 
For let the product of AB, BC, as we said, be square, and CA even, and let CA be bisected by D. 
       
It is then manifest that the square product of AB, BC together with the square on CD is equal to the square on BD. [See :ref:`X.28.l.1 <X.28.l.1>`] 
Let the unit DE be subtracted; therefore the product of AB, BC together with the square on CE is less than the square on BD. 
I say then that the square product of AB, BC together with the square on CE will not be square. 
For, if it is square, it is either equal to the square on BE, or less than the square on BE, but cannot any more be greater, lest the unit be divided. 
First, if possible, let the product of AB, BC together with the square on CE be equal to the square on BE, and let GA be double of the unit DE. 
Since then the whole AC is double of the whole CD, and in them AG is double of DE, therefore the remainder GC is also double of the remainder EC; therefore GC is bisected by E. 
Therefore the product of GB, BC together with the square on CE is equal to the square on BE. [:ref:`II.6 <II.6>`] 
But the product of AB, BC together with the square on CE is also, by hypothesis, equal to the square on BE; therefore the product of GB, BC together with the square on CE is equal to the product of AB, BC together with the square on CE. 
And, if the common square on CE be subtracted, it follows that AB is equal to GB: which is absurd. 
Therefore the product of AB, BC together with the square on CE is not equal to the square on BE. 
I say next that neither is it less than the square on BE. 
For, if possible, let it be equal to the square on BF, and let HA be double of DF. 
Now it will again follow that HC is double of CF; so that CH has also been bisected at F, and for this reason the product of HB, BC together with the square on FC is equal to the square on BF. [:ref:`II.6 <II.6>`] 
But, by hypothesis, the product of AB, BC together with the square on CE is also equal to the square on BF. 
Thus the product of HB, BC together with the square on CF will also be equal to the product of AB, BC together with the square on CE: which is absurd. 
Therefore the product of AB, BC together with the square on CE is not less than the square on BE. 
And it was proved that neither is it equal to the square on BE. 
Therefore the product of AB, BC together with the square on CE is not square. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     bgcolor="black";
     node [shape=box, style="rounded,filled", fontname="Helvetica", color="white", fontcolor="white"];
     edge [color="white", fontcolor="white"];
     rankdir="TB";
     "V.15" [fillcolor="#222244", URL="/heath/V/15/", target="_top"];
     "I.32" [fillcolor="#222244", URL="/heath/I/32/", target="_top"];
     "VII.18" [fillcolor="#222244", URL="/heath/VII/18/", target="_top"];
     "X.28.l.1" [fillcolor="#333333"];
     "I.cn.1" [fillcolor="#442222", URL="/heath/I/cn.1/", target="_top"];
     "VII.2" [fillcolor="#222244", URL="/heath/VII/2/", target="_top"];
     "I.28" [fillcolor="#222244", URL="/heath/I/28/", target="_top"];
     "V.23" [fillcolor="#222244", URL="/heath/V/23/", target="_top"];
     "I.41" [fillcolor="#222244", URL="/heath/I/41/", target="_top"];
     "I.43" [fillcolor="#222244", URL="/heath/I/43/", target="_top"];
     "VII.27" [fillcolor="#222244", URL="/heath/VII/27/", target="_top"];
     "X.def.3" [fillcolor="#224422", URL="/heath/X/def.3/", target="_top"];
     "I.post.4" [fillcolor="#444422", URL="/heath/I/post.4/", target="_top"];
     "VI.2" [fillcolor="#222244", URL="/heath/VI/2/", target="_top"];
     "I.7" [fillcolor="#222244", URL="/heath/I/7/", target="_top"];
     "V.def.4" [fillcolor="#224422", URL="/heath/V/def.4/", target="_top"];
     "V.20" [fillcolor="#222244", URL="/heath/V/20/", target="_top"];
     "VII.5" [fillcolor="#222244", URL="/heath/VII/5/", target="_top"];
     "VII.34" [fillcolor="#222244", URL="/heath/VII/34/", target="_top"];
     "I.37" [fillcolor="#222244", URL="/heath/I/37/", target="_top"];
     "I.cn.3" [fillcolor="#442222", URL="/heath/I/cn.3/", target="_top"];
     "I.post.2" [fillcolor="#444422", URL="/heath/I/post.2/", target="_top"];
     "VII.6" [fillcolor="#222244", URL="/heath/VII/6/", target="_top"];
     "VII.15" [fillcolor="#222244", URL="/heath/VII/15/", target="_top"];
     "I.def.23" [fillcolor="#224422", URL="/heath/I/def.23/", target="_top"];
     "IX.26" [fillcolor="#222244", URL="/heath/IX/26/", target="_top"];
     "I.35" [fillcolor="#222244", URL="/heath/I/35/", target="_top"];
     "VII.def.7" [fillcolor="#224422", URL="/heath/VII/def.7/", target="_top"];
     "VII.23" [fillcolor="#222244", URL="/heath/VII/23/", target="_top"];
     "VII.def.12" [fillcolor="#224422", URL="/heath/VII/def.12/", target="_top"];
     "VII.def.21" [fillcolor="#224422", URL="/heath/VII/def.21/", target="_top"];
     "V.13" [fillcolor="#222244", URL="/heath/V/13/", target="_top"];
     "VI.11" [fillcolor="#222244", URL="/heath/VI/11/", target="_top"];
     "I.post.3" [fillcolor="#444422", URL="/heath/I/post.3/", target="_top"];
     "VIII.1" [fillcolor="#222244", URL="/heath/VIII/1/", target="_top"];
     "III.20" [fillcolor="#222244", URL="/heath/III/20/", target="_top"];
     "VI.17" [fillcolor="#222244", URL="/heath/VI/17/", target="_top"];
     "VII.35" [fillcolor="#222244", URL="/heath/VII/35/", target="_top"];
     "VII.14" [fillcolor="#222244", URL="/heath/VII/14/", target="_top"];
     "V.def.5" [fillcolor="#224422", URL="/heath/V/def.5/", target="_top"];
     "I.31" [fillcolor="#222244", URL="/heath/I/31/", target="_top"];
     "I.3" [fillcolor="#222244", URL="/heath/I/3/", target="_top"];
     "I.def.10" [fillcolor="#224422", URL="/heath/I/def.10/", target="_top"];
     "X.11" [fillcolor="#222244", URL="/heath/X/11/", target="_top"];
     "I.13" [fillcolor="#222244", URL="/heath/I/13/", target="_top"];
     "VI.18" [fillcolor="#222244", URL="/heath/VI/18/", target="_top"];
     "X.def.4" [fillcolor="#224422", URL="/heath/X/def.4/", target="_top"];
     "I.cn.2" [fillcolor="#442222", URL="/heath/I/cn.2/", target="_top"];
     "X.28" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/X/28/", target="_top"];
     "I.9" [fillcolor="#222244", URL="/heath/I/9/", target="_top"];
     "VII.13" [fillcolor="#222244", URL="/heath/VII/13/", target="_top"];
     "VI.19.p.1" [fillcolor="#333333"];
     "VIII.2.p.1" [fillcolor="#333333"];
     "II.6" [fillcolor="#222244", URL="/heath/II/6/", target="_top"];
     "VI.14" [fillcolor="#222244", URL="/heath/VI/14/", target="_top"];
     "I.post.5" [fillcolor="#444422", URL="/heath/I/post.5/", target="_top"];
     "VII.25" [fillcolor="#222244", URL="/heath/VII/25/", target="_top"];
     "I.4" [fillcolor="#222244", URL="/heath/I/4/", target="_top"];
     "VIII.22" [fillcolor="#222244", URL="/heath/VIII/22/", target="_top"];
     "VI.4" [fillcolor="#222244", URL="/heath/VI/4/", target="_top"];
     "X.13" [fillcolor="#222244", URL="/heath/X/13/", target="_top"];
     "VI.12" [fillcolor="#222244", URL="/heath/VI/12/", target="_top"];
     "V.9" [fillcolor="#222244", URL="/heath/V/9/", target="_top"];
     "VII.def.6" [fillcolor="#224422", URL="/heath/VII/def.6/", target="_top"];
     "VI.1" [fillcolor="#222244", URL="/heath/VI/1/", target="_top"];
     "VII.24" [fillcolor="#222244", URL="/heath/VII/24/", target="_top"];
     "X.12" [fillcolor="#222244", URL="/heath/X/12/", target="_top"];
     "I.46" [fillcolor="#222244", URL="/heath/I/46/", target="_top"];
     "VII.20" [fillcolor="#222244", URL="/heath/VII/20/", target="_top"];
     "I.post.1" [fillcolor="#444422", URL="/heath/I/post.1/", target="_top"];
     "I.16" [fillcolor="#222244", URL="/heath/I/16/", target="_top"];
     "I.29" [fillcolor="#222244", URL="/heath/I/29/", target="_top"];
     "VII.2.p.1" [fillcolor="#333333"];
     "I.cn.4" [fillcolor="#442222", URL="/heath/I/cn.4/", target="_top"];
     "I.47" [fillcolor="#222244", URL="/heath/I/47/", target="_top"];
     "I.14" [fillcolor="#222244", URL="/heath/I/14/", target="_top"];
     "V.7.p.1" [fillcolor="#333333"];
     "V.3" [fillcolor="#222244", URL="/heath/V/3/", target="_top"];
     "V.17" [fillcolor="#222244", URL="/heath/V/17/", target="_top"];
     "VII.33" [fillcolor="#222244", URL="/heath/VII/33/", target="_top"];
     "X.6" [fillcolor="#222244", URL="/heath/X/6/", target="_top"];
     "V.def.9" [fillcolor="#224422", URL="/heath/V/def.9/", target="_top"];
     "IX.1" [fillcolor="#222244", URL="/heath/IX/1/", target="_top"];
     "V.18" [fillcolor="#222244", URL="/heath/V/18/", target="_top"];
     "V.14" [fillcolor="#222244", URL="/heath/V/14/", target="_top"];
     "VIII.8" [fillcolor="#222244", URL="/heath/VIII/8/", target="_top"];
     "I.27" [fillcolor="#222244", URL="/heath/I/27/", target="_top"];
     "I.2" [fillcolor="#222244", URL="/heath/I/2/", target="_top"];
     "VII.26" [fillcolor="#222244", URL="/heath/VII/26/", target="_top"];
     "VII.4" [fillcolor="#222244", URL="/heath/VII/4/", target="_top"];
     "I.26" [fillcolor="#222244", URL="/heath/I/26/", target="_top"];
     "VIII.20" [fillcolor="#222244", URL="/heath/VIII/20/", target="_top"];
     "V.4" [fillcolor="#222244", URL="/heath/V/4/", target="_top"];
     "I.8" [fillcolor="#222244", URL="/heath/I/8/", target="_top"];
     "V.16" [fillcolor="#222244", URL="/heath/V/16/", target="_top"];
     "VII.9" [fillcolor="#222244", URL="/heath/VII/9/", target="_top"];
     "I.5" [fillcolor="#222244", URL="/heath/I/5/", target="_top"];
     "VII.17" [fillcolor="#222244", URL="/heath/VII/17/", target="_top"];
     "VI.22" [fillcolor="#222244", URL="/heath/VI/22/", target="_top"];
     "V.11" [fillcolor="#222244", URL="/heath/V/11/", target="_top"];
     "VIII.18" [fillcolor="#222244", URL="/heath/VIII/18/", target="_top"];
     "X.5" [fillcolor="#222244", URL="/heath/X/5/", target="_top"];
     "I.11" [fillcolor="#222244", URL="/heath/I/11/", target="_top"];
     "III.21" [fillcolor="#222244", URL="/heath/III/21/", target="_top"];
     "I.36" [fillcolor="#222244", URL="/heath/I/36/", target="_top"];
     "I.38" [fillcolor="#222244", URL="/heath/I/38/", target="_top"];
     "VII.16" [fillcolor="#222244", URL="/heath/VII/16/", target="_top"];
     "X.8" [fillcolor="#222244", URL="/heath/X/8/", target="_top"];
     "III.31" [fillcolor="#222244", URL="/heath/III/31/", target="_top"];
     "VII.19" [fillcolor="#222244", URL="/heath/VII/19/", target="_top"];
     "I.33" [fillcolor="#222244", URL="/heath/I/33/", target="_top"];
     "V.8" [fillcolor="#222244", URL="/heath/V/8/", target="_top"];
     "VII.3" [fillcolor="#222244", URL="/heath/VII/3/", target="_top"];
     "V.10" [fillcolor="#222244", URL="/heath/V/10/", target="_top"];
     "I.17" [fillcolor="#222244", URL="/heath/I/17/", target="_top"];
     "V.22" [fillcolor="#222244", URL="/heath/V/22/", target="_top"];
     "V.12" [fillcolor="#222244", URL="/heath/V/12/", target="_top"];
     "X.22" [fillcolor="#222244", URL="/heath/X/22/", target="_top"];
     "I.15" [fillcolor="#222244", URL="/heath/I/15/", target="_top"];
     "VI.8.p.1" [fillcolor="#333333"];
     "VII.def.20" [fillcolor="#224422", URL="/heath/VII/def.20/", target="_top"];
     "I.39" [fillcolor="#222244", URL="/heath/I/39/", target="_top"];
     "X.7" [fillcolor="#222244", URL="/heath/X/7/", target="_top"];
     "VII.1" [fillcolor="#222244", URL="/heath/VII/1/", target="_top"];
     "IX.24" [fillcolor="#222244", URL="/heath/IX/24/", target="_top"];
     "VI.13" [fillcolor="#222244", URL="/heath/VI/13/", target="_top"];
     "X.21" [fillcolor="#222244", URL="/heath/X/21/", target="_top"];
     "VI.def.1" [fillcolor="#224422", URL="/heath/VI/def.1/", target="_top"];
     "I.1" [fillcolor="#222244", URL="/heath/I/1/", target="_top"];
     "I.23" [fillcolor="#222244", URL="/heath/I/23/", target="_top"];
     "VII.21" [fillcolor="#222244", URL="/heath/VII/21/", target="_top"];
     "V.7" [fillcolor="#222244", URL="/heath/V/7/", target="_top"];
     "VII.12" [fillcolor="#222244", URL="/heath/VII/12/", target="_top"];
     "I.10" [fillcolor="#222244", URL="/heath/I/10/", target="_top"];
     "VII.22" [fillcolor="#222244", URL="/heath/VII/22/", target="_top"];
     "III.22" [fillcolor="#222244", URL="/heath/III/22/", target="_top"];
     "VIII.2" [fillcolor="#222244", URL="/heath/VIII/2/", target="_top"];
     "V.def.7" [fillcolor="#224422", URL="/heath/V/def.7/", target="_top"];
     "IV.1" [fillcolor="#222244", URL="/heath/IV/1/", target="_top"];
     "VII.def.15" [fillcolor="#224422", URL="/heath/VII/def.15/", target="_top"];
     "VIII.3" [fillcolor="#222244", URL="/heath/VIII/3/", target="_top"];
     "VI.16" [fillcolor="#222244", URL="/heath/VI/16/", target="_top"];
     "X.18.l.1" [fillcolor="#333333"];
     "V.2" [fillcolor="#222244", URL="/heath/V/2/", target="_top"];
     "V.1" [fillcolor="#222244", URL="/heath/V/1/", target="_top"];
     "VII.10" [fillcolor="#222244", URL="/heath/VII/10/", target="_top"];
     "VIII.4" [fillcolor="#222244", URL="/heath/VIII/4/", target="_top"];
     "V.21" [fillcolor="#222244", URL="/heath/V/21/", target="_top"];
     "I.34" [fillcolor="#222244", URL="/heath/I/34/", target="_top"];
     "I.def.15" [fillcolor="#224422", URL="/heath/I/def.15/", target="_top"];
     "X.23" [fillcolor="#222244", URL="/heath/X/23/", target="_top"];
     "V.16" -> "V.15";
     "V.23" -> "V.15";
     "VI.1" -> "V.15";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "VI.18" -> "I.32";
     "VII.19" -> "VII.18";
     "VIII.2" -> "VII.18";
     "X.28" -> "X.28.l.1";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "VI.4" -> "I.28";
     "V.16" -> "V.23";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "II.6" -> "I.43";
     "VIII.2" -> "VII.27";
     "VIII.3" -> "VII.27";
     "X.23" -> "X.def.3";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "VI.4" -> "VI.2";
     "VI.11" -> "VI.2";
     "VI.12" -> "VI.2";
     "I.8" -> "I.7";
     "V.8" -> "V.def.4";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "VII.6" -> "VII.5";
     "VII.9" -> "VII.5";
     "VII.10" -> "VII.5";
     "VII.12" -> "VII.5";
     "VIII.4" -> "VII.34";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "VII.9" -> "VII.6";
     "VII.10" -> "VII.6";
     "VII.12" -> "VII.6";
     "VII.16" -> "VII.15";
     "I.27" -> "I.def.23";
     "X.28" -> "IX.26";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "IX.26" -> "VII.def.7";
     "VII.24" -> "VII.23";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "VII.23" -> "VII.def.12";
     "VII.24" -> "VII.def.12";
     "VIII.18" -> "VII.def.21";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "VI.22" -> "VI.11";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "VIII.2" -> "VIII.1";
     "III.21" -> "III.20";
     "X.28" -> "VI.17";
     "VIII.4" -> "VII.35";
     "VIII.1" -> "VII.14";
     "VIII.8" -> "VII.14";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "I.46" -> "I.31";
     "II.6" -> "I.31";
     "VI.11" -> "I.31";
     "VI.12" -> "I.31";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VI.11" -> "I.3";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "X.21" -> "X.11";
     "X.22" -> "X.11";
     "X.23" -> "X.11";
     "X.28" -> "X.11";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "VI.22" -> "VI.18";
     "X.21" -> "X.def.4";
     "X.22" -> "X.def.4";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "I.10" -> "I.9";
     "VII.14" -> "VII.13";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "VIII.18" -> "VII.13";
     "VIII.20" -> "VII.13";
     "VI.22" -> "VI.19.p.1";
     "X.6" -> "VI.19.p.1";
     "VIII.3" -> "VIII.2.p.1";
     "X.28" -> "II.6";
     "VI.16" -> "VI.14";
     "X.22" -> "VI.14";
     "I.29" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VII.27" -> "VII.25";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "IX.1" -> "VIII.22";
     "VI.18" -> "VI.4";
     "X.22" -> "X.13";
     "X.23" -> "X.13";
     "VI.22" -> "VI.12";
     "X.28" -> "VI.12";
     "VI.2" -> "V.9";
     "VI.14" -> "V.9";
     "VI.22" -> "V.9";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "IX.24" -> "VII.def.6";
     "VI.2" -> "VI.1";
     "VI.14" -> "VI.1";
     "X.21" -> "VI.1";
     "X.23" -> "VI.1";
     "VII.25" -> "VII.24";
     "VII.26" -> "VII.24";
     "X.13" -> "X.12";
     "I.47" -> "I.46";
     "II.6" -> "I.46";
     "VII.21" -> "VII.20";
     "VII.24" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.1" -> "VII.20";
     "VIII.4" -> "VII.20";
     "VIII.8" -> "VII.20";
     "VIII.20" -> "VII.20";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "VII.3" -> "VII.2.p.1";
     "I.4" -> "I.cn.4";
     "X.13" -> "I.47";
     "I.47" -> "I.14";
     "VI.14" -> "I.14";
     "X.5" -> "V.7.p.1";
     "X.6" -> "V.7.p.1";
     "V.4" -> "V.3";
     "V.16" -> "V.17";
     "V.18" -> "V.17";
     "VII.34" -> "VII.33";
     "VIII.3" -> "VII.33";
     "VIII.8" -> "VII.33";
     "VIII.20" -> "VII.33";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "X.12" -> "X.6";
     "VIII.18" -> "V.def.9";
     "X.28" -> "IX.1";
     "V.16" -> "V.18";
     "V.16" -> "V.14";
     "V.18" -> "V.14";
     "IX.1" -> "VIII.8";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "I.3" -> "I.2";
     "VII.27" -> "VII.26";
     "VII.20" -> "VII.4";
     "I.34" -> "I.26";
     "VIII.22" -> "VIII.20";
     "V.22" -> "V.4";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "V.23" -> "V.16";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "X.28" -> "V.16";
     "VII.10" -> "VII.9";
     "I.7" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.22" -> "VII.17";
     "VII.34" -> "VII.17";
     "VIII.2" -> "VII.17";
     "VIII.18" -> "VII.17";
     "VIII.20" -> "VII.17";
     "IX.1" -> "VII.17";
     "X.22" -> "VI.22";
     "V.16" -> "V.11";
     "V.18" -> "V.11";
     "V.23" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.14" -> "V.11";
     "VI.22" -> "V.11";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "IX.1" -> "VIII.18";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "X.12" -> "X.5";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "III.22" -> "III.21";
     "I.38" -> "I.36";
     "II.6" -> "I.36";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.24" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "X.11" -> "X.8";
     "VI.13" -> "III.31";
     "X.13" -> "III.31";
     "VII.24" -> "VII.19";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "I.36" -> "I.33";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "VII.33" -> "VII.3";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "III.31" -> "I.17";
     "VI.4" -> "I.17";
     "V.16" -> "V.22";
     "VI.4" -> "V.22";
     "VI.22" -> "V.22";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "V.15" -> "V.12";
     "X.23" -> "X.22";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "VI.13" -> "VI.8.p.1";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "VIII.4" -> "VII.def.20";
     "VIII.8" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "VI.2" -> "I.39";
     "X.11" -> "X.7";
     "VII.2" -> "VII.1";
     "IX.26" -> "IX.24";
     "X.28" -> "IX.24";
     "X.28" -> "VI.13";
     "X.22" -> "X.21";
     "X.23" -> "X.21";
     "X.28" -> "X.21";
     "VI.18" -> "VI.def.1";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.31" -> "I.23";
     "VI.18" -> "I.23";
     "VII.24" -> "VII.21";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "VIII.1" -> "VII.21";
     "VIII.8" -> "VII.21";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.14" -> "V.7";
     "VII.19" -> "V.7";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "I.16" -> "I.10";
     "VIII.2" -> "VII.22";
     "VIII.3" -> "VII.22";
     "III.31" -> "III.22";
     "VIII.3" -> "VIII.2";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "X.13" -> "IV.1";
     "VII.22" -> "VII.def.15";
     "VII.24" -> "VII.def.15";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "VIII.8" -> "VIII.3";
     "VI.17" -> "VI.16";
     "X.28" -> "VI.16";
     "X.23" -> "X.18.l.1";
     "V.3" -> "V.2";
     "V.17" -> "V.2";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.17" -> "V.1";
     "VII.13" -> "VII.10";
     "X.12" -> "VIII.4";
     "V.16" -> "V.21";
     "V.23" -> "V.21";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.46" -> "I.34";
     "VI.4" -> "I.34";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "X.28" -> "X.23";
   }



Required for
------------

:ref:`X.104`, :ref:`X.110`, :ref:`X.75`, :ref:`X.81`, :ref:`X.93`, :ref:`X.99`