...
For, since the straight line ``CB`` has been cut at random at ``D``\ , :raw-html-m2r:`<span class="center">the squares on `CB`, `BD` are equal to twice the rectangle contained by `CB`, `BD` and the square on `DC`. [<a href="/elem.2.7">II. 7</a>]</span>`
...
But the square on ``AB`` is equal to the squares on ``BD``\ , ``DA``\ , for the angle at ``D`` is right; [\ :raw-html-m2r:`<a href="/elem.1.47">I. 47</a>`\ ] and the square on ``AC`` is equal to the squares on ``AD``\ , ``DC``\ ; therefore the squares on ``CB``\ , ``BA`` are equal to the square on ``AC`` and twice the rectangle ``CB``\ , ``BD``\ ,
