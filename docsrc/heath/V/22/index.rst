...
.. picture:: V.22.graphic.inverted.png
...
.. container:: center
   ...
   therefore, as ``G`` is to ``K``, so is ``H`` to ``L``. [:ref:`V.4 <V.4>`]
...
Since, then, there are three magnitudes ``G``, ``K``, ``M``, and others ``H``, ``L``, ``N`` equal to them in multitude, which taken two and two together are in the same ratio, therefore, ex aequali, if ``G`` is in excess of ``M``, ``H`` is also in excess of ``N``; if equal, equal; and if less, less. [:ref:`V.20 <V.20>`]
...
Therefore, as ``A`` is to ``C``, so is ``D`` to ``F``. [:ref:`V.def.5 <V.def.5>`]
...
.. graphviz::
   ...
   digraph {
     ...
     "V.22" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/V/22/", target="_top"];
