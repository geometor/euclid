:order: 8
:number: 579
:type: prop
:dependencies: V.12, VI.20, XI.24, XI.28, XI.33, XII.7




.. picture:: XII.8.graphic.inverted.png

.. _XII.8:

XII.8
=====

   Similar pyramids which have triangular bases are in the triplicate ratio of their corresponding sides.

Let there be similar and similarly situated pyramids of which the triangles ABC, DEF, are the bases and the points G, H the vertices; I say that the pyramid ABCG has to the pyramid DEFH the ratio triplicate of that which BC has to EF.

For let the parallelepipedal solids BGML, EHQP be completed.

Now, since the pyramid ABCG is similar to the pyramid DEFH, therefore the angle ABC is equal to the angle DEF, the angle GBC to the angle HEF, and the angle ABG to the angle DEH; and, as AB is to DE, so is BC to EF, and BG to EH.

And since, as AB is to DE, so is BC to EF, and the sides are proportional about equal angles, therefore the parallelogram BM is similar to the parallelogram EQ.

For the same reason BN is also similar to ER, and BK to EO; therefore the three parallelograms MB, BK, BN are similar to the three EQ, EO, ER.

But the three parallelograms MB, BK, BN are equal and similar to their three opposites, and the three EQ, EO, ER are equal and similar to their three opposites. [:ref:`XI.24 <XI.24>`]

Therefore the solids BGML, EHQP are contained by similar planes equal in multitude.

Therefore the solid BGML is similar to the solid EHQP.

But similar parallelepipedal solids are in the triplicate ratio of their corresponding sides. [:ref:`XI.33 <XI.33>`]

Therefore the solid BGML has to the solid EHQP the ratio triplicate of that which the corresponding side BC has to the corresponding side EF.

But, as the solid BGML is to the solid EHQP, so is the pyramid ABCG to the pyramid DEFH, inasmuch as the pyramid is a sixth part of the solid, because the prism which is half of the parallelepipedal solid [:ref:`XI.28 <XI.28>`] is also triple of the pyramid. [:ref:`XII.7 <XII.7>`]

Therefore the pyramid ABCG also has to the pyramid DEFH the ratio triplicate of that which BC has to EF. Q. E. D.


.. _XII.8.p.1:


**XII.8.p.1**


From this it is manifest that similar pyramids which have polygonal bases are also to one another in the triplicate ratio of their corresponding sides. 
For, if they are divided into the pyramids contained in them which have triangular bases, by virtue of the fact that the similar polygons forming their bases are also divided into similar triangles equal in multitude and corresponding to the wholes [:ref:`VI.20 <VI.20>`], then, as the one pyramid which has a triangular base in the one complete pyramid is to the one pyramid which has a triangular base in the other complete pyramid, so also will all the pyramids which have triangular bases contained in the one pyramid be to all the pyramids which have triangular bases contained in the other pyramid [:ref:`V.12 <V.12>`], that is, the pyramid itself which has a polygonal base to the pyramid which has a polygonal base. 
But the pyramid which has a triangular base is to the pyramid which has a triangular base in the triplicate ratio of the corresponding sides; therefore also the pyramid which has a polygonal base has to the pyramid which has a similar base the ratio triplicate of that which the side has to the side.

PORISM.

From this it is manifest that similar pyramids which have polygonal bases are also to one another in the triplicate ratio of their corresponding sides. 

For, if they are divided into the pyramids contained in them which have triangular bases, by virtue of the fact that the similar polygons forming their bases are also divided into similar triangles equal in multitude and corresponding to the wholes [:ref:`VI.20 <VI.20>`], then, as the one pyramid which has a triangular base in the one complete pyramid is to the one pyramid which has a triangular base in the other complete pyramid, so also will all the pyramids which have triangular bases contained in the one pyramid be to all the pyramids which have triangular bases contained in the other pyramid [:ref:`V.12 <V.12>`], that is, the pyramid itself which has a polygonal base to the pyramid which has a polygonal base. 

But the pyramid which has a triangular base is to the pyramid which has a triangular base in the triplicate ratio of the corresponding sides; therefore also the pyramid which has a polygonal base has to the pyramid which has a similar base the ratio triplicate of that which the side has to the side.


Dependency Graph
----------------

.. graphviz::

   digraph {
     bgcolor="black";
     node [shape=box, style="rounded,filled", fontname="Helvetica", color="white", fontcolor="white"];
     edge [color="white", fontcolor="white"];
     rankdir="TB";
     "V.15" [fillcolor="#222244", URL="/heath/V/15/", target="_top"];
     "XI.7" [fillcolor="#222244", URL="/heath/XI/7/", target="_top"];
     "XI.8" [fillcolor="#222244", URL="/heath/XI/8/", target="_top"];
     "V.def.9" [fillcolor="#224422", URL="/heath/V/def.9/", target="_top"];
     "I.32" [fillcolor="#222244", URL="/heath/I/32/", target="_top"];
     "I.cn.1" [fillcolor="#442222", URL="/heath/I/cn.1/", target="_top"];
     "V.18" [fillcolor="#222244", URL="/heath/V/18/", target="_top"];
     "V.14" [fillcolor="#222244", URL="/heath/V/14/", target="_top"];
     "XI.5" [fillcolor="#222244", URL="/heath/XI/5/", target="_top"];
     "I.28" [fillcolor="#222244", URL="/heath/I/28/", target="_top"];
     "XI.def.10" [fillcolor="#224422", URL="/heath/XI/def.10/", target="_top"];
     "I.27" [fillcolor="#222244", URL="/heath/I/27/", target="_top"];
     "V.23" [fillcolor="#222244", URL="/heath/V/23/", target="_top"];
     "I.2" [fillcolor="#222244", URL="/heath/I/2/", target="_top"];
     "I.41" [fillcolor="#222244", URL="/heath/I/41/", target="_top"];
     "I.43" [fillcolor="#222244", URL="/heath/I/43/", target="_top"];
     "I.post.4" [fillcolor="#444422", URL="/heath/I/post.4/", target="_top"];
     "XI.33" [fillcolor="#222244", URL="/heath/XI/33/", target="_top"];
     "XI.4" [fillcolor="#222244", URL="/heath/XI/4/", target="_top"];
     "I.26" [fillcolor="#222244", URL="/heath/I/26/", target="_top"];
     "I.42" [fillcolor="#222244", URL="/heath/I/42/", target="_top"];
     "I.8" [fillcolor="#222244", URL="/heath/I/8/", target="_top"];
     "XI.24" [fillcolor="#222244", URL="/heath/XI/24/", target="_top"];
     "V.4" [fillcolor="#222244", URL="/heath/V/4/", target="_top"];
     "V.16" [fillcolor="#222244", URL="/heath/V/16/", target="_top"];
     "VI.2" [fillcolor="#222244", URL="/heath/VI/2/", target="_top"];
     "I.7" [fillcolor="#222244", URL="/heath/I/7/", target="_top"];
     "I.5" [fillcolor="#222244", URL="/heath/I/5/", target="_top"];
     "XI.16" [fillcolor="#222244", URL="/heath/XI/16/", target="_top"];
     "VI.22" [fillcolor="#222244", URL="/heath/VI/22/", target="_top"];
     "V.11" [fillcolor="#222244", URL="/heath/V/11/", target="_top"];
     "V.def.4" [fillcolor="#224422", URL="/heath/V/def.4/", target="_top"];
     "V.20" [fillcolor="#222244", URL="/heath/V/20/", target="_top"];
     "I.11" [fillcolor="#222244", URL="/heath/I/11/", target="_top"];
     "I.37" [fillcolor="#222244", URL="/heath/I/37/", target="_top"];
     "I.cn.3" [fillcolor="#442222", URL="/heath/I/cn.3/", target="_top"];
     "I.36" [fillcolor="#222244", URL="/heath/I/36/", target="_top"];
     "I.38" [fillcolor="#222244", URL="/heath/I/38/", target="_top"];
     "I.post.2" [fillcolor="#444422", URL="/heath/I/post.2/", target="_top"];
     "XII.8" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/XII/8/", target="_top"];
     "XI.17" [fillcolor="#222244", URL="/heath/XI/17/", target="_top"];
     "I.def.23" [fillcolor="#224422", URL="/heath/I/def.23/", target="_top"];
     "V.def.11" [fillcolor="#224422", URL="/heath/V/def.11/", target="_top"];
     "I.35" [fillcolor="#222244", URL="/heath/I/35/", target="_top"];
     "VI.19" [fillcolor="#222244", URL="/heath/VI/19/", target="_top"];
     "XII.5" [fillcolor="#222244", URL="/heath/XII/5/", target="_top"];
     "I.30" [fillcolor="#222244", URL="/heath/I/30/", target="_top"];
     "I.post.3" [fillcolor="#444422", URL="/heath/I/post.3/", target="_top"];
     "V.13" [fillcolor="#222244", URL="/heath/V/13/", target="_top"];
     "VI.11" [fillcolor="#222244", URL="/heath/VI/11/", target="_top"];
     "I.33" [fillcolor="#222244", URL="/heath/I/33/", target="_top"];
     "V.8" [fillcolor="#222244", URL="/heath/V/8/", target="_top"];
     "XI.31" [fillcolor="#222244", URL="/heath/XI/31/", target="_top"];
     "XII.7" [fillcolor="#222244", URL="/heath/XII/7/", target="_top"];
     "I.45" [fillcolor="#222244", URL="/heath/I/45/", target="_top"];
     "XI.39" [fillcolor="#222244", URL="/heath/XI/39/", target="_top"];
     "V.10" [fillcolor="#222244", URL="/heath/V/10/", target="_top"];
     "I.17" [fillcolor="#222244", URL="/heath/I/17/", target="_top"];
     "V.def.5" [fillcolor="#224422", URL="/heath/V/def.5/", target="_top"];
     "XII.2.l.1" [fillcolor="#333333"];
     "V.22" [fillcolor="#222244", URL="/heath/V/22/", target="_top"];
     "I.31" [fillcolor="#222244", URL="/heath/I/31/", target="_top"];
     "VI.15" [fillcolor="#222244", URL="/heath/VI/15/", target="_top"];
     "I.3" [fillcolor="#222244", URL="/heath/I/3/", target="_top"];
     "XI.32" [fillcolor="#222244", URL="/heath/XI/32/", target="_top"];
     "V.12" [fillcolor="#222244", URL="/heath/V/12/", target="_top"];
     "XI.def.3" [fillcolor="#224422", URL="/heath/XI/def.3/", target="_top"];
     "I.def.10" [fillcolor="#224422", URL="/heath/I/def.10/", target="_top"];
     "I.15" [fillcolor="#222244", URL="/heath/I/15/", target="_top"];
     "I.39" [fillcolor="#222244", URL="/heath/I/39/", target="_top"];
     "I.13" [fillcolor="#222244", URL="/heath/I/13/", target="_top"];
     "VI.6" [fillcolor="#222244", URL="/heath/VI/6/", target="_top"];
     "VI.18" [fillcolor="#222244", URL="/heath/VI/18/", target="_top"];
     "XI.3" [fillcolor="#222244", URL="/heath/XI/3/", target="_top"];
     "I.cn.2" [fillcolor="#442222", URL="/heath/I/cn.2/", target="_top"];
     "XI.1" [fillcolor="#222244", URL="/heath/XI/1/", target="_top"];
     "I.9" [fillcolor="#222244", URL="/heath/I/9/", target="_top"];
     "VI.19.p.1" [fillcolor="#333333"];
     "XII.3" [fillcolor="#222244", URL="/heath/XII/3/", target="_top"];
     "I.post.5" [fillcolor="#444422", URL="/heath/I/post.5/", target="_top"];
     "VI.def.1" [fillcolor="#224422", URL="/heath/VI/def.1/", target="_top"];
     "I.1" [fillcolor="#222244", URL="/heath/I/1/", target="_top"];
     "I.23" [fillcolor="#222244", URL="/heath/I/23/", target="_top"];
     "VI.20" [fillcolor="#222244", URL="/heath/VI/20/", target="_top"];
     "V.def.10" [fillcolor="#224422", URL="/heath/V/def.10/", target="_top"];
     "I.4" [fillcolor="#222244", URL="/heath/I/4/", target="_top"];
     "VI.4" [fillcolor="#222244", URL="/heath/VI/4/", target="_top"];
     "V.7" [fillcolor="#222244", URL="/heath/V/7/", target="_top"];
     "VI.12" [fillcolor="#222244", URL="/heath/VI/12/", target="_top"];
     "V.9" [fillcolor="#222244", URL="/heath/V/9/", target="_top"];
     "VI.1" [fillcolor="#222244", URL="/heath/VI/1/", target="_top"];
     "I.10" [fillcolor="#222244", URL="/heath/I/10/", target="_top"];
     "XII.4" [fillcolor="#222244", URL="/heath/XII/4/", target="_top"];
     "XI.30" [fillcolor="#222244", URL="/heath/XI/30/", target="_top"];
     "V.def.7" [fillcolor="#224422", URL="/heath/V/def.7/", target="_top"];
     "XI.28" [fillcolor="#222244", URL="/heath/XI/28/", target="_top"];
     "I.post.1" [fillcolor="#444422", URL="/heath/I/post.1/", target="_top"];
     "XI.25" [fillcolor="#222244", URL="/heath/XI/25/", target="_top"];
     "XI.29" [fillcolor="#222244", URL="/heath/XI/29/", target="_top"];
     "I.16" [fillcolor="#222244", URL="/heath/I/16/", target="_top"];
     "I.29" [fillcolor="#222244", URL="/heath/I/29/", target="_top"];
     "XI.9" [fillcolor="#222244", URL="/heath/XI/9/", target="_top"];
     "I.cn.4" [fillcolor="#442222", URL="/heath/I/cn.4/", target="_top"];
     "I.14" [fillcolor="#222244", URL="/heath/I/14/", target="_top"];
     "V.2" [fillcolor="#222244", URL="/heath/V/2/", target="_top"];
     "V.1" [fillcolor="#222244", URL="/heath/V/1/", target="_top"];
     "V.21" [fillcolor="#222244", URL="/heath/V/21/", target="_top"];
     "I.34" [fillcolor="#222244", URL="/heath/I/34/", target="_top"];
     "XI.10" [fillcolor="#222244", URL="/heath/XI/10/", target="_top"];
     "I.44" [fillcolor="#222244", URL="/heath/I/44/", target="_top"];
     "I.def.15" [fillcolor="#224422", URL="/heath/I/def.15/", target="_top"];
     "V.3" [fillcolor="#222244", URL="/heath/V/3/", target="_top"];
     "V.17" [fillcolor="#222244", URL="/heath/V/17/", target="_top"];
     "XI.2" [fillcolor="#222244", URL="/heath/XI/2/", target="_top"];
     "XI.6" [fillcolor="#222244", URL="/heath/XI/6/", target="_top"];
     "V.16" -> "V.15";
     "V.23" -> "V.15";
     "VI.1" -> "V.15";
     "XI.8" -> "XI.7";
     "XI.9" -> "XI.8";
     "VI.19" -> "V.def.9";
     "VI.6" -> "I.32";
     "VI.18" -> "I.32";
     "VI.20" -> "I.32";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.30" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "I.44" -> "I.cn.1";
     "I.45" -> "I.cn.1";
     "V.16" -> "V.18";
     "V.16" -> "V.14";
     "V.18" -> "V.14";
     "XI.6" -> "XI.5";
     "VI.4" -> "I.28";
     "XI.6" -> "I.28";
     "XI.28" -> "XI.def.10";
     "XI.31" -> "XI.def.10";
     "XI.33" -> "XI.def.10";
     "XII.3" -> "XI.def.10";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.16" -> "V.23";
     "I.3" -> "I.2";
     "I.42" -> "I.41";
     "VI.1" -> "I.41";
     "I.44" -> "I.43";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "XII.8" -> "XI.33";
     "XI.5" -> "XI.4";
     "XI.8" -> "XI.4";
     "XI.9" -> "XI.4";
     "I.34" -> "I.26";
     "XI.4" -> "I.26";
     "I.44" -> "I.42";
     "I.45" -> "I.42";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "XI.4" -> "I.8";
     "XI.6" -> "I.8";
     "XI.10" -> "I.8";
     "XI.29" -> "I.8";
     "XI.25" -> "XI.24";
     "XI.31" -> "XI.24";
     "XI.33" -> "XI.24";
     "XII.8" -> "XI.24";
     "V.22" -> "V.4";
     "V.23" -> "V.16";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "VI.19" -> "V.16";
     "XII.4" -> "V.16";
     "XII.5" -> "V.16";
     "VI.4" -> "VI.2";
     "VI.11" -> "VI.2";
     "VI.12" -> "VI.2";
     "XI.17" -> "VI.2";
     "XII.3" -> "VI.2";
     "I.8" -> "I.7";
     "I.7" -> "I.5";
     "XI.17" -> "XI.16";
     "XI.24" -> "XI.16";
     "XII.4" -> "VI.22";
     "V.16" -> "V.11";
     "V.18" -> "V.11";
     "V.23" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.6" -> "V.11";
     "VI.15" -> "V.11";
     "VI.19" -> "V.11";
     "VI.22" -> "V.11";
     "XI.17" -> "V.11";
     "XI.31" -> "V.11";
     "V.8" -> "V.def.4";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.13" -> "I.11";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "I.38" -> "I.36";
     "XI.29" -> "I.36";
     "I.42" -> "I.38";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "XII.4" -> "XI.17";
     "I.27" -> "I.def.23";
     "XI.16" -> "I.def.23";
     "VI.19" -> "V.def.11";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "XI.31" -> "I.35";
     "VI.20" -> "VI.19";
     "XII.7" -> "XII.5";
     "I.45" -> "I.30";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "VI.19" -> "VI.11";
     "VI.22" -> "VI.11";
     "I.36" -> "I.33";
     "I.45" -> "I.33";
     "XI.10" -> "I.33";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "XI.32" -> "XI.31";
     "XI.39" -> "XI.31";
     "XII.8" -> "XII.7";
     "XI.32" -> "I.45";
     "XII.3" -> "XI.39";
     "XII.4" -> "XI.39";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "VI.4" -> "I.17";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "XI.25" -> "V.def.5";
     "XII.5" -> "XII.2.l.1";
     "V.16" -> "V.22";
     "VI.4" -> "V.22";
     "VI.20" -> "V.22";
     "VI.22" -> "V.22";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "I.42" -> "I.31";
     "I.44" -> "I.31";
     "VI.11" -> "I.31";
     "VI.12" -> "I.31";
     "VI.19" -> "VI.15";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VI.11" -> "I.3";
     "XI.33" -> "XI.32";
     "XII.4" -> "XI.32";
     "V.15" -> "V.12";
     "VI.20" -> "V.12";
     "XII.4" -> "V.12";
     "XII.8" -> "V.12";
     "XI.4" -> "XI.def.3";
     "XI.5" -> "XI.def.3";
     "XI.6" -> "XI.def.3";
     "XI.8" -> "XI.def.3";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "I.44" -> "I.15";
     "XI.4" -> "I.15";
     "VI.2" -> "I.39";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "VI.20" -> "VI.6";
     "VI.22" -> "VI.18";
     "XI.5" -> "XI.3";
     "XI.7" -> "XI.3";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.45" -> "I.cn.2";
     "XI.2" -> "XI.1";
     "XI.16" -> "XI.1";
     "I.10" -> "I.9";
     "VI.22" -> "VI.19.p.1";
     "XII.4" -> "XII.3";
     "XII.5" -> "XII.3";
     "I.29" -> "I.post.5";
     "I.44" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VI.18" -> "VI.def.1";
     "VI.20" -> "VI.def.1";
     "XII.3" -> "VI.def.1";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.31" -> "I.23";
     "I.42" -> "I.23";
     "VI.6" -> "I.23";
     "VI.18" -> "I.23";
     "XI.31" -> "I.23";
     "XII.8" -> "VI.20";
     "XI.33" -> "V.def.10";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "VI.6" -> "I.4";
     "XI.4" -> "I.4";
     "XI.6" -> "I.4";
     "XI.24" -> "I.4";
     "XI.29" -> "I.4";
     "XII.3" -> "I.4";
     "VI.6" -> "VI.4";
     "VI.18" -> "VI.4";
     "VI.20" -> "VI.4";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.15" -> "V.7";
     "XI.31" -> "V.7";
     "VI.22" -> "VI.12";
     "VI.2" -> "V.9";
     "VI.6" -> "V.9";
     "VI.15" -> "V.9";
     "VI.22" -> "V.9";
     "XI.31" -> "V.9";
     "VI.2" -> "VI.1";
     "VI.15" -> "VI.1";
     "VI.19" -> "VI.1";
     "VI.20" -> "VI.1";
     "XI.33" -> "VI.1";
     "I.16" -> "I.10";
     "XII.5" -> "XII.4";
     "XI.31" -> "XI.30";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "XI.39" -> "XI.28";
     "XII.8" -> "XI.28";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "XI.31" -> "XI.25";
     "XI.32" -> "XI.25";
     "XI.30" -> "XI.29";
     "XI.31" -> "XI.29";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "I.30" -> "I.29";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.44" -> "I.29";
     "I.45" -> "I.29";
     "XI.8" -> "I.29";
     "XII.3" -> "I.29";
     "XI.10" -> "XI.9";
     "I.4" -> "I.cn.4";
     "I.45" -> "I.14";
     "VI.15" -> "I.14";
     "V.3" -> "V.2";
     "V.17" -> "V.2";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.17" -> "V.1";
     "V.16" -> "V.21";
     "V.23" -> "V.21";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.45" -> "I.34";
     "VI.4" -> "I.34";
     "XI.24" -> "I.34";
     "XI.28" -> "I.34";
     "XI.29" -> "I.34";
     "XI.39" -> "I.34";
     "XII.3" -> "I.34";
     "XII.7" -> "I.34";
     "XI.24" -> "XI.10";
     "XII.3" -> "XI.10";
     "I.45" -> "I.44";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "V.4" -> "V.3";
     "V.16" -> "V.17";
     "V.18" -> "V.17";
     "XI.6" -> "XI.2";
     "XI.8" -> "XI.2";
     "XI.9" -> "XI.6";
   }



Required for
------------

:ref:`XII.12`