...
PROP_END_RE = re.compile(
    r"(?:being\s+)?what\s+it\s+was\s+required\s+to\s+(?:do|prove)|Q\.?\s*E\.?\s*[DF]\.?",
    re.IGNORECASE,
)


def get_book_from_path(path: Path) -> str:
...
def process_folder(folder_path: Path, volume: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines):
            ...
            if not start_match and line_stripped.upper() == "PROPOSITION":
                for j in range(i + 1, len(lines)):
                    ...
                    if next_line:
                        num_match = re.match(
                            r"^([IVXLCDM\d]+)\.?$", next_line, re.IGNORECASE
                        )
                        if num_match:
...
def run_analysis() -> None:
    ...
    for root, dirs, files in os.walk(EXTRACTED_DIR):
        ...
        if (is_prop_folder or is_book_folder) and has_txt:
            ...
            if "volume_iii" in str(root_path).lower():
                vol = "III"
            elif "volume_ii" in str(root_path).lower():
                vol = "II"
            elif "volume_i" in str(root_path).lower():
                vol = "I"
