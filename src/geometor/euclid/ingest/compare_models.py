"""Module for comparing generic model outputs for graphic detection."""
...
def detect_graphic_with_usage(
    model_name: str, image_path: Path
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """Detects graphics using a specified AI model and returns result with usage stats."""
    try:
        ...
        result = {"x_offset": xmin, "y_offset": ymin, "crop_box": [width, height]}
...
def run_comparison() -> None:
    """Runs a comparison of graphic detection models for a target set of propositions."""
    print("Loading index...")
    ...
    for prop_id in TARGET_PROPS:
        ...
        for model_name in TARGET_MODELS:
            ...
            if "total_tokens" in usage:
                ...
                total_usage[model_name]["candidates"] += usage.get(
                    "candidates_tokens", 0
                )
            ...
            if "error" not in result:
                ...
                report_lines.append(
                    f"  Diff (AI - GT): x={d_x:+d}, y={d_y:+d}, w={d_w:+d}, h={d_h:+d}"
                )
    ...
    for model_name in TARGET_MODELS:
        ...
        report_lines.append(
            f"{model_name}: {u['total']} total tokens (Prompt: {u['prompt']}, Output: {u['candidates']})"
        )
