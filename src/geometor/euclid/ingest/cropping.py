"""Module for automated cropping of proposition images."""
...
def process_propositions() -> None:
    ...
    manual_instructions_path = (
        RESOURCES_DIR / "heath_manual" / "cropping_instructions.json"
    )
    manual_instructions = {}
    ...
    def get_book_numeral(book_str: Optional[str]) -> str:
    ...
    for prop in propositions:
        ...
        for i, page_num in enumerate(page_range):
            ...
            if not rel_img_path:
                print(
                    f"  Missing image for {prop_key} page {page_num} (key: {lookup_key})"
                )
                cropped_images = []
                break
            ...
            try:
                ...
                box = calculate_crop_box(
                    img.height, current_total_lines, p_start_line, p_end_line
                )
        ...
        if len(cropped_images) > 1:
            ...
            stitched_image = Image.new(
                "RGB", (CROP_WIDTH, total_height), (255, 255, 255)
            )
            current_y = 0
        ...
        if prop_key in manual_instructions:
            ...
            if all([gx is not None, gy is not None, g_box is not None]):
                ...
                if (
                    graphic_box[2] <= stitched_image.width
                    and graphic_box[3] <= stitched_image.height
                ):
                    graphic_img = stitched_image.crop(graphic_box)
                    graphic_img.save(CROPPED_DIR / f"{prop_key}.graphic.png")


def run_cropping() -> None:
    """Runs the cropping process for all propositions."""
