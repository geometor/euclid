"""Module for detecting graphics in proposition pages."""
...
def save_json(path: Path, data: Any) -> None:  # noqa: ANN401
    """Saves data to a JSON file."""
    with open(path, "w") as f:
        json.dump(data, f, indent=2)


def get_book_slug_from_path(path: Path) -> Optional[str]:
    """Extracts the book slug from a file path."""
...
def detect_graphic_cv(image_path: Path, debug: bool = False) -> Dict[str, Any]:
    """Detects graphics in an image using OpenCV algorithms."""
    # Load image
    ...
    # Thresholds
    MIN_GRAPHIC_DIM = (
        60  # Minimum width or height to be considered a "line" or "circle" part
    )
    ...
    for i in range(1, num_labels):
        ...
        if (search_x <= cx <= search_x + search_w) and (
            search_y <= cy <= search_y + search_h
        ):
    ...
    return {"x_offset": int(gx), "y_offset": int(gy), "crop_box": [int(gw), int(gh)]}


def process_books(
    target_book: Optional[str] = None, dry_run: bool = False, debug: bool = False
) -> None:
    """Processes books to detect and update graphic coordinates."""
    ...
    for index_path in index_files:
        ...
        if target_book:
            ...
            if t_book == b_slug:
                match = True
            if f"book_{t_book}" == b_slug:
                match = True
            if f"book{t_book}" == b_slug:
                match = True
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Scan and update graphic coordinates using OpenCV."
    )
    parser.add_argument(
        "--book", "-b", type=str, help="Specific book to scan (e.g., 'I' or 'book_i')."
    )
    parser.add_argument("--dry-run", action="store_true", help="Do not save changes")
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Save debug images to debug_graphics/ folder",
    )
