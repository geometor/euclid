"""Module for extracting images and text from PDF volumes."""
...
def extract_volume(vol_key: str, relative_path: str) -> None:
    ...
    if existing_images:
        try:
            ...
            with Image.open(existing_images[0]) as img:
                ...
                if width > 1200:
                    print(
                        f"  Detected upscaled images (width {width}). Re-extracting at default resolution..."
                    )
                    should_extract = True
                else:
                    print(
                        f"  Found {len(existing_images)} existing images with acceptable resolution. Skipping."
                    )
    ...
    if should_extract:
        ...
        cmd_image = ["pdftoppm", "-png", str(pdf_path), str(vol_output_dir / prefix)]
...
def generate_manifest() -> None:
    """Generates manifests of extracted pages for each volume."""
    ...
    for vol_key in VOLUMES.keys():
        ...
        for img_path in images:
            ...
            try:
                ...
                pages.append({"page_num": page_num, "image": rel_img, "text": rel_txt})
            except ValueError:
                continue
...
def extract_toc_from_pdf(pdf_path: Path) -> Optional[str]:
    """Extracts the table of contents from a PDF using pdftohtml."""
    cmd = [
        "pdftohtml",
        "-xml",
        "-stdout",
        "-i",  # ignore images
        "-l",
        "1",  # process only first page (sufficient for outline)
        str(pdf_path),
    ]
...
def parse_outline_recursive(outline_element: ET.Element) -> List[Dict[str, Any]]:
    """Recursively parses XML outline elements."""
    entries = []
    last_entry = None
...
def parse_outline_xml(xml_string: str) -> List[Dict[str, Any]]:
    """Parses the XML outline string into a structured list."""
    try:
...
def run_toc_extraction() -> None:
    """Runs TOC extraction for all configured volumes."""
    full_toc = {}
    ...
    for vol_key, rel_path in VOLUMES.items():
        ...
        if xml_content:
            ...
            full_toc[f"volume_{vol_key}"] = (
                toc  # Keep for consistency with original behavior if needed elsewhere
            )
...
def run_extraction() -> None:
    """Runs the full extraction process: images/text, manifests, and TOC."""
