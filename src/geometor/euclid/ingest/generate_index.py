"""Module for generating the proposition index from resources."""
...
def normalize_label(label: str) -> str:
    """Normalizes a proposition label (e.g. 'I' -> '1')."""
...
def find_image_path(base_dir: Path, volume: str, page_num: int) -> Optional[Path]:
    """Finds the relative path to a page image."""
...
def get_book_roman(book_slug: str) -> str:
    """Converts book slug to Roman numeral."""
...
def generate_index() -> None:
    """Generates the main proposition index JSON file."""
    ...
    for item in items:
        ...
        if crop_info:
            ...
            for p in range(item["start_page"], item["end_page"] + 1):
                ...
                entry["pages"].append(
                    {"page": p, "file": str(rel_path) if rel_path else None}
                )
