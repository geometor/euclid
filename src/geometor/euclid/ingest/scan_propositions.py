"""Module for scanning propositions from text files and generating indexes."""
...
def get_book_and_volume_from_path(
    file_path: Path,
) -> Tuple[Optional[str], Optional[str]]:
    """Extracts volume key and book slug from a given file path."""
...
def scan_propositions(target_book: Optional[str] = None) -> None:
    """Scans text files for propositions and generates index files.

    Args:
        target_book: Optional book identifier to filter scanning.
    """
    print("""
    ----------------------------------------------------------------------------------------------------
    ...
    for (vol_key, book_slug), files in files_by_book.items():
        ...
        for f_path in files:
            page_num = (
                int(re.search(r"(\d{4})\.txt$", f_path.name).group(1))
                if re.search(r"(\d{4})\.txt$", f_path.name)
                else 0
            )
            try:
        ...
        for f_idx, f_path in enumerate(files):
            ...
            for i, line in enumerate(lines):
                ...
                is_proposition_start = re.match(
                    r"^PROPOSITION\s*$", line_stripped, re.IGNORECASE
                )
                is_proposition_end = re.match(
                    r"^Q\.\s*E\.\s*D\.$|^Q\.\s*E\.\s*F\.$|^\(Being\) what it was required to do\.$",
                    line_stripped,
                    re.IGNORECASE,
                )
                ...
                if is_proposition_start:
                    ...
                    if current_prop and current_prop["status"] == "open":
                        ...
                        current_prop["end_line_text"] = (
                            lines[i - 1].strip() if i > 0 else ""
                        )
                        current_prop["status"] = "closed_by_next_prop_start"

                        cut_y = (
                            int(PAGE_HEADER_END + (i / total_lines) * BODY_HEIGHT)
                            if total_lines > 0
                            else PAGE_FOOTER_START
                        )
                        end_offset_val = DEFAULT_PAGE_HEIGHT - cut_y

                        if (
                            current_prop["pages"]
                            and current_prop["pages"][-1]["page"] == page_num
                        ):
                    ...
                    cut_y = (
                        int(PAGE_HEADER_END + ((i + 1) / total_lines) * BODY_HEIGHT)
                        if total_lines > 0
                        else PAGE_FOOTER_START
                    )
                    end_offset_val = DEFAULT_PAGE_HEIGHT - cut_y
            ...
            if current_prop and current_prop["status"] == "open":
                if (
                    not current_prop["pages"]
                    or current_prop["pages"][-1]["page"] != page_num
                ):
                        ...
                        current_prop["pages"].append(
                            {
                                ...
                                "start_offset_px": PAGE_HEADER_END,
                                "end_offset_px": DEFAULT_PAGE_HEIGHT
                                - PAGE_FOOTER_START,
                            }
                        )
        ...
        if current_prop and current_prop["status"] == "open":
            ...
            last_page_num = int(
                re.search(r"(\d{4})\.txt$", last_file_in_book.name).group(1)
            )
            ...
            if (
                current_prop["pages"]
                and current_prop["pages"][-1]["page"] == last_page_num
            ):
                current_prop["pages"][-1]["end_offset_px"] = end_offset_val
            else:
                start_offset_px = PAGE_HEADER_END
                if current_prop["start_page"] == last_page_num:
                    start_y = (
                        int(
                            PAGE_HEADER_END
                            + (current_prop["start_line_index"] / len(last_lines))
                            * BODY_HEIGHT
                        )
                        if len(last_lines) > 0
                        else PAGE_HEADER_END
    ...
    PLACEHOLDER_GRAPHIC = {"x_offset": 335, "y_offset": 130, "crop_box": [300, 200]}
    ...
    for (vol_key, book_slug), props_list in propositions_by_book.items():
        ...
        print(
            f"Generated {output_index_path} with {len(final_propositions_list)} propositions."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-b",
        type=str,
        help="Specific book to scan (e.g., 'I' or 'book_i'). If omitted, scans all found books.",
    )
