"""Module for stitching cropped proposition images into single images."""
...
def get_book_slug_from_path(path: Path) -> Optional[str]:
    """Extracts the book slug from a given path."""
...
def stitch_propositions(target_book: Optional[str] = None) -> None:
    """Stitches cropped images for each proposition into a single image.

    Args:
        target_book: Optional book identifier to filter stitching.
    """
    ...
    if not index_files:
        print(
            "No index.json files found in book directories. Did you run scan_propositions.py?"
        )
        return
        ...
        for prop in propositions:
            ...
            if "graphics" in prop and prop["graphics"]:
                ...
                if gx is not None and gy is not None and g_box:
                    ...
                    if (
                        gx + gw <= stitched_image.width
                        and gy + gh <= stitched_image.height
                    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Stitch propositions from scanned indexes."
    )
    parser.add_argument(
        "--book",
        "-b",
        type=str,
        help="Specific book to stitch (e.g., 'I' or 'book_i').",
    )
