"""Module for checking references in enunciations."""
...
def find_refs_in_enunciation(file_path: str) -> Tuple[Optional[str], Optional[str]]:
    """Finds reference tags in the enunciation of a file."""
    ...
    if book_num == 1:
        enunc_match = re.search(
            r'<div4 type="Enunc".*?>(.*?)</div4>', content, re.DOTALL
        )
        if enunc_match:
...
def main() -> None:
    """Main function to check references in books 1-6."""
    ...
    for i in range(1, 7):  # Books 1 to 6
        ...
        propositions = re.findall(
            r'(<div3 id="elem\.(\d+)\.(\d+)"[^>]*>.*?</div3>)', book_content, re.DOTALL
        )
        ...
        for prop_content, book_num_str, prop_num_str in propositions:
            ...
            if int(book_num_str) == 1:
                enunc_match = re.search(
                    r'<div4 type="Enunc"[^>]*>(.*?)</div4>', prop_content, re.DOTALL
                )
