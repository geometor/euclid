"""Module for transforming XML books into RST documentation."""
...
def get_taxonomy(enunciation_text: str) -> tuple[list[str], list[str]]:
    """Extracts categories and tags from enunciation text."""
    categories = []
    tags = []
...
def parse_book_xml(
    ...
    """Parses a book XML file and returns its data structure."""
...
def generate_proof_chain_dot(
    graph: nx.DiGraph, element_ref: str, ref_to_path_map: dict[str, tuple[str, str]]
) -> str:
    """Generates a GraphViz DOT string for the proof chain of an element."""
...
def generate_rst_files(
    ...
    output_dir: str | Path,
    graph: nx.DiGraph,
    ref_to_path_map: dict[str, tuple[str, str]],
    metadata: dict,
) -> None:
    """Generates RST files for a book and its entries."""
        ...
        for entry in section["entries"]:
            if entry["folder_name"]:
                ...
                if canonical_images_dir.exists() and entry["canonical_ref"]:
                    ...
                    image_files.extend(
                        sorted(canonical_images_dir.glob(f"{image_stem}-*.jpg"))
                    )
                ...
                if heath_propositions_dir.exists() and entry["canonical_ref"]:
                    ...
                    graphic_file = (
                        heath_propositions_dir / f"{graphic_stem}.graphic.png"
                    )
                    ...
                    if graphic_file.exists():
                        ...
                        inverted_img_path = (
                            entry_dir / f"{graphic_file.stem}.inverted.png"
                        )
                ...
                if entry.get("analysis"):
                    ...
                    if analysis["dependencies"]:
                        ...
                        dot_content = generate_proof_chain_dot(
                            graph, entry["canonical_ref"], ref_to_path_map
                        )
                        if dot_content:
...
def generate_dependency_graph(
    graph: nx.DiGraph, output_dir: str | Path = "docsrc/elements2"
) -> None:
    """Generates a full dependency graph in DOT format."""
        ...
        for section in book_data["sections"]:
            for entry in section["entries"]:
                if entry["canonical_ref"] and entry["folder_name"]:
                    ref_to_path_map[entry["canonical_ref"]] = (
                        book_roman,
                        entry["folder_name"],
                    )
