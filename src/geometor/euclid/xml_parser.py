"""Module for parsing XML elements into dictionary structures."""
...
def parse_element_xml(file_path: Path) -> Dict[str, Any]:
    ...
    if not enunciation and not proof:
        ...
        if all_paragraphs:
            ...
            if len(all_paragraphs) > 1:
                proof_paragraphs = [
                    _convert_element_to_rst(p) for p in all_paragraphs[1:]
                ]
                proof = "\n\n".join(proof_paragraphs)
